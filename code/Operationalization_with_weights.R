...
# import comission data
comission_data <- read_xlsx("C:/Users/nirko/Dropbox/Civil Servants' Perceptions of Democratic Decline/Commission data/workers_profile_1.4.23.xlsx")
# Filter population data to include only תיכון or בכיר
filtered_comission_data <- comission_data[comission_data$מדרג %in% c("תיכון", "בכיר"), ]
# Replace Hebrew labels with "male" and "female"
filtered_comission_data$gender <- ifelse(
  filtered_comission_data$מגדר == "עובד", "male", "female"
)
...
# Gender
pop_gender <- prop.table(table(filtered_comission_data$gender))

# Age
pop_age <- prop.table(table(filtered_comission_data$`טווח גיל`))

# Ranking
filtered_comission_data$ranking_standardized <- ifelse(
  filtered_comission_data$מדרג == "תיכון", "middle", "senior_or_very_senior"
)
pop_ranking <- prop.table(table(filtered_comission_data$ranking_standardized))

# Tenure (bin numeric values into ranges) for the filtered dataset
filtered_comission_data$tenure_group <- cut(
  as.numeric(sub("\\..*", "", filtered_comission_data$`ותק בשירות המדינה`)),
  breaks = c(-Inf, 1, 5, 10, 20, Inf),
  labels = c("1-", "1-5", "6-10", "11-20", "20+"),
  right = FALSE
)
pop_tenure <- prop.table(table(filtered_comission_data$tenure_group))

#2. Align Rankings
# Map population rankings: תיכון → "middle", בכיר → "senior_or_very_senior"
filtered_comission_data$ranking_standardized <- ifelse(
  filtered_comission_data$מדרג == "תיכון", "middle", "senior_or_very_senior"
)

# Map sample rankings: middle → "middle", senior/very senior → "senior_or_very_senior"
...
# Bin tenure into categories for population data
filtered_comission_data$tenure_group <- cut(
  as.numeric(sub("\\..*", "", filtered_comission_data$`ותק בשירות המדינה`)),
  breaks = c(-Inf, 1, 5, 10, 20, Inf),
  labels = c("1-", "1-5", "6-10", "11-20", "20+"),
...
#calculate weights
# Gender Weight
weights_gender <- pop_gender / sample_gender

# Age Weight
weights_age <- pop_age / sample_age

# Ranking Weight
weights_ranking <- pop_ranking / sample_ranking

# Tenure Weight
weights_tenure <- pop_tenure / sample_tenure

#5. Weights to Observations
# Assign weights to the filtered sample data
filtered_sample_data$weight_gender <- as.numeric(filtered_sample_data$weight_gender)
filtered_sample_data$weight_age <- as.numeric(filtered_sample_data$weight_age)
filtered_sample_data$weight_ranking <- as.numeric(filtered_sample_data$weight_ranking)
filtered_sample_data$weight_tenure <- as.numeric(filtered_sample_data$weight_tenure)

#6. Combine Weights
# Combine weights for each observation
filtered_sample_data$combined_weight <- with(filtered_sample_data,
                                             weight_gender * weight_age * weight_ranking * weight_tenure
)
